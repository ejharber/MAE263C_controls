...
class velocity_controller_node(object):
    ...
    def transform_velocity(self, v, w):
        ...
        p_finger_sensor = np.array([32.15, 8.19, 7.27])/1000
        R_finger_sensor = Rotation.from_rotvec([0, 0, 12*np.pi/180]).as_matrix()
        print(R_finger_sensor)

        try:
            (p, R) = velocity_node.listener.lookupTransform('j2s7s300_link_finger_tip_1', 'root', rospy.Time(0))
            print(p)
        ...
        # try:
        #     (p, R) = velocity_node.listener.lookupTransform( 'j2s7s300_link_finger_tip_1','j2s7s300_end_effector', rospy.Time(0))
...
if __name__ == '__main__': 
    try:
        ...
        while not rospy.is_shutdown():
            ...
            #     v_goal, w_goal = velocity_node.transform_velocity([0,0,0], [0.5,0,0])
            ...
            #     v_goal, w_goal = velocity_node.transform_velocity([0,0,0], [0,0,0.5])
            ...
            if counter < 100:
                ...
                v_goal, w_goal = velocity_node.transform_velocity([0.5,0,0], [0,0,0])
                ...
                v_goal, w_goal = velocity_node.transform_velocity([0,0.5,0], [0,0,0])
                ...
                v_goal, w_goal = velocity_node.transform_velocity([0,0,0.5], [0,0,0])
